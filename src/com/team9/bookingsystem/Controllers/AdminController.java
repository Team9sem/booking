...
public class AdminController {
	...
	@FXML
	private GridPane searchOptions;
	@FXML
	private GridPane roomOptions;
	@FXML
	private UserSearchController searchOptionsController;
	@FXML
	private Label loginLabel;
	@FXML
	private Label searchPreferences;
	@FXML
	private Label searchFor;
	@FXML
	private ToggleButton userToggle;
	@FXML
	private ToggleButton roomToggle;
	@FXML
	private GridPane userSearchGridPane;
	@FXML
	private GridPane roomSearchGridPane;
	@FXML
	...
	@FXML
	public void commitChanges(ActionEvent event) {
...
//        userToggle.setSelected(true);
	...
	private void setupRoomTableView() {
		...
		location.setOnEditCommit(new EventHandler<TableColumn.CellEditEvent<Room, String>>() {
			...
			public void handle(TableColumn.CellEditEvent<Room, String> event) {
				...
				Room updatedRoom = event.getTableView().getItems().get(event.getTablePosition().getRow());
		...
		buttons.setCellValueFactory(
				new Callback<TableColumn.CellDataFeatures<Room, Boolean>, ObservableValue<Boolean>>() {
					@Override
	...
	private void setupUserTableView() {
				...
				getIsAvailable.setOnSucceeded(new EventHandler<WorkerStateEvent>() {
					...
					public void handle(WorkerStateEvent serviceEvent) {
						...
						if (isAvailableUserName) {
							...
							User updatedUser = event.getTableView().getItems().get(event.getTablePosition().getRow());
		...
		// TableColumn projector = new TableColumn("Projector");
		// projector.setCellFactory(TextFieldTableCell.forTableColumn(new
		// IntegerStringConverter()));
		// projector.setOnEditCommit(new
		// EventHandler<TableColumn.CellEditEvent<Room,Integer>>() {
		// @Override
		// public void handle(TableColumn.CellEditEvent<Room,Integer> event) {
		//
		// if(event.getNewValue() == 1 || event.getNewValue() == 0)
		...
		// });
		//
		// projector.setCellValueFactory(new
		// PropertyValueFactory<Room,Integer>("hasProjector"));
		// projector.setPrefWidth(10.0);
		//
		// TableColumn whiteboard = new TableColumn("WhiteBoard");
		// whiteboard.setCellFactory(TextFieldTableCell.forTableColumn(new
		// IntegerStringConverter()));
		// whiteboard.setOnEditCommit(new
		// EventHandler<TableColumn.CellEditEvent<Room,Integer>>() {
		// @Override
		// public void handle(TableColumn.CellEditEvent<Room,Integer> event) {
		//
		...
		// }
		// });
		// whiteboard.setCellValueFactory(new
		// PropertyValueFactory<Room,Integer>("hasWhiteboard"));
		// whiteboard.setPrefWidth(10.0);
		//
		// TableColumn coffeMachine = new TableColumn("CoffeeMachine");
		// coffeMachine.setCellFactory(TextFieldTableCell.forTableColumn(new
		// IntegerStringConverter()));
		// coffeMachine.setOnEditCommit(new
		// EventHandler<TableColumn.CellEditEvent<Room,Integer>>() {
		// @Override
		// public void handle(TableColumn.CellEditEvent<Room,Integer> event) {
		//
		...
		// }
		// });
		// coffeMachine.setCellValueFactory(new
		// PropertyValueFactory<Room,Integer>("hasCoffeeMachine"));
		// coffeMachine.setPrefWidth(10.0);
		...
		userTableView.getColumns().addAll(userId, username, firstName, lastName, passWord, userType, street, zip,
				pNumberCol, buttons);
		userTableView.setColumnResizePolicy(new CustomColumnResizePolicy());
		// userTableView.setColumnResizePolicy(new
